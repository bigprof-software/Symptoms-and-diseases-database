<?php
	...
	// begin sending emails
		...
		if($sendToAll){
			$res=sql("select email from membership_users");
		}elseif($isGroup){
			$res=sql("select email from membership_users where groupID='$groupID'");
		}else{
			$res=sql("select email from membership_users where lcase(memberID)='$memberID'");
		}
